...
This is the ONLY service that touches the database.
"""
...
async def lifespan(app: FastAPI):
    await get_pool()          # eagerly create the pool at startup
    yield
    await close_pool()
...
# ---------------------------------------------------------------------------
# Credit Score & History Endpoints (score_analysis, score_improvement, score_trend, score_trend_summary)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Bills & Payments (bill_payment, bbps_refund)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Disputes (dispute_closed_active, dispute_fake)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Loans (loan_eligibility, loan_listing)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Report Update, NOC Draft, EMI Conversion (report_update, noc_mail_draft, overdue_emi_conversion)
# ---------------------------------------------------------------------------
...
async def draft_noc_letter(req: NocDraftRequest):
    await _jitter()
    pool = await get_pool()
    cust = await pool.fetchrow("SELECT * FROM customers WHERE customer_id = $1", req.customer_id)
...
I, {cust_name}, am writing with reference to my loan account number {req.account_number} maintained with {req.lender}.
...
Kindly dispatch the NOC to my registered address or send a soft copy to {cust['email'] if cust else 'my email'}.
